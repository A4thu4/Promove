...
from backend.app.core.logic import calcular_matriz_carreira, verificar_evolucao
...
class TestLogic(unittest.TestCase):
    def test_basic_evolution_promove(self):
        ...
        carreira = calcular_matriz_carreira(
            is_ueg=False,
            data_inicio=data_inicio,
            data_enquadramento=date(2022, 1, 1),
            afastamentos=[],
            aperfeicoamentos=[(date(2022, 2, 1), 10.0)], # 10h = 0.9 pts
            titulacoes=[],
            resp_unicas=[],
            resp_mensais=[],
            dados_tit={}
        ...
        # Debug: print(carreira[17]) # Linha do mês 18 (2023-06-01)
        
        result = verificar_evolucao(False, "A", carreira, data_inicio)
    ...
    def test_evolution_with_resp_mensal(self):
        ...
        carreira = calcular_matriz_carreira(
            ...
            data_enquadramento=date(2022, 1, 1),
            afastamentos=[],
            aperfeicoamentos=[(date(2022, 1, 1), 100.0)], # 9 pts
            titulacoes=[],
            resp_unicas=[],
    ...
    def test_evolution_ueg(self):
        ...
        result = verificar_evolucao(True, "A", carreira, data_inicio)
        ...
        resp_mensais = [("C. Comissão: DAS1", date(2022, 1, 1), date(2024, 1, 1), 1.0)]
        carreira_ok = calcular_matriz_carreira(
            is_ueg=True,
            data_inicio=data_inicio,
            data_enquadramento=date(2022, 1, 1),
            afastamentos=[],
        ...
        result_ok = verificar_evolucao(True, "A", carreira_ok, data_inicio)
